...
app.get('/browse', (req, res) => {
  ...
  const mediaFiles = files.filter(f => {
    ...
    return fs.statSync(full).isFile() && ['.jpg', '.jpeg', '.png', ".webp", '.mp4', '.webm'].includes(ext);
  });
  ...
  const backLink = relDir
    ...
    const slideshowBtn = `
  <button
    style="font-size:50px;padding:10px;margin:10px 0;"
...
app.get('/zip', (req, res) => {
  ...
  const entries = zip.getEntries().filter(e => !e.isDirectory && /\.(jpg|jpeg|png|webp|mp4|webm)$/i.test(e.entryName));
